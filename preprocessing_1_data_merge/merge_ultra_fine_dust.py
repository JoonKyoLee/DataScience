...
# 경보단계에 우선순위 추가
merged_ultra_fine_dust['priority'] = merged_ultra_fine_dust['경보단계'].map(priority)

# '일자'와 '권역'을 기준으로 그룹화하여 우선순위가 높은 행을 선택
merged_ultra_fine_dust = merged_ultra_fine_dust.sort_values('priority', ascending=False).drop_duplicates(subset=['일자', '권역'])

# 'priority' 컬럼 삭제
merged_ultra_fine_dust = merged_ultra_fine_dust.drop(columns=['priority'])
